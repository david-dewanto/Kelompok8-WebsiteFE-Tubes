        ...
        <div className="relative">
          <div className="relative">
            <textarea
              ...
              className={`w-full min-h-[200px] p-6 pr-16 rounded-2xl bg-white/50 dark:bg-gray-800/50 backdrop-blur-lg border-2 ${
                error
                  ? 'border-danger-500 focus:border-danger-600'
                  : 'border-gray-200 dark:border-gray-700 focus:border-primary-500'
              } focus:outline-none focus:ring-4 focus:ring-primary-500/20 transition-all duration-300 sequence-input resize-none`}
              disabled={isLoading}
              spellCheck={false}
              autoComplete="off"
              autoCorrect="off"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {sampleSequences.map((sample, index) => (
              <motion.button
                ...
                className="text-left p-4 rounded-xl bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 transition-all duration-300 group"
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
              >
                <div className="font-semibold text-gray-800 dark:text-gray-200 group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">
                  {sample.name}
                </div>
